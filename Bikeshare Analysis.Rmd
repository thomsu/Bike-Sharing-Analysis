...
```{r}
packages <- c("dplyr", "DBI", "skimr", "stringr", "tidyr", "tibble", "lubridate", "geosphere", "ggplot2", "ggmap", "ggthemes", "gridExtra", "gganimate", "ggalluvial", "modeest")
...
basemap <- ggmap(get_stamenmap(c(left = min(top_member_start_station_df$start_lng) - .05,
                  bottom = min(top_member_start_station_df$start_lat) - .01,
                  right = max(top_member_start_station_df$start_lng) + .05,
                  top = max(top_member_start_station_df$start_lat)) + .01,
...
basemap <- ggmap(get_stamenmap(c(left = min(top_member_end_station_df$end_lng) - .05,
                  bottom = min(top_member_end_station_df$end_lat) - .01,
                  right = max(top_member_end_station_df$end_lng) + .05,
                  top = max(top_member_end_station_df$end_lat)) + .01,
                  maptype = "toner-lite", zoom = 13))
...
animate(route_time_lapse, duration = 25, height = 800, width = 600)
...
freq_routes_df <- bikeshare_df %>%
  ...
  group_by(member_casual, route_name) %>% 
  summarize(subgroup_route_count = length(route_name)) %>% 
  ungroup() %>% 
  with_groups(route_name, mutate, route_count = sum(subgroup_route_count)) %>% 
  slice_max(na.omit(route_count), n = 30) %>%  
  mutate(axis1 = str_extract(route_name, "^[A-Za-z0-9& ]+(?= - )"), axis2 = str_extract(route_name, "(?<= - )[A-Za-z0-9& ]+$"))

freq_routes_df
```

```{r}
ggplot(freq_routes_df, aes(y = subgroup_route_count, axis1 = axis1, axis2 = axis2)) +
  geom_alluvium(aes(fill = member_casual), width = 1/12) +
  geom_stratum(width = 1/12, fill = "grey", color = "white") +
  geom_label(stat = "stratum", aes(label = str_wrap(after_stat(stratum), 25)), size = 3, fill = "light grey") +
  scale_fill_manual(values = c("coral", "turquoise")) +
  ylab(" ") +
  guides(fill=guide_legend(title="Rider Type")) + 
  labs(title = "Most Frequent Bike Routes (Bidirectional)") +
  theme(axis.text.x = element_blank(), axis.text.y = element_blank(), panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.y = element_blank(), panel.grid.major.y = element_blank())
```
...
freq_routes_df <- bikeshare_df %>%
  ...
  summarize(subgroup_route_count = length(month_of_year), route_count = first(route_count), .groups = "keep") %>%
  ungroup() %>%
...
```{r}
ggplot(freq_routes_df, aes(y = route_name, x = subgroup_route_count, fill = member_casual, group = member_casual)) +
  geom_bar(position="dodge", stat="identity") + 
  facet_grid(month_of_year ~ ., scales = "free", space = "free") +
